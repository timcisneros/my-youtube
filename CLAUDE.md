...
- **Player lifecycle and media timelines**: Every player load has a generation; unload, destroy, or a newer load invalidates stale asynchronous work before it can replace the active provider. Explicit VOD start times are clamped against manifest duration and applied after MSE metadata is usable. HLS fMP4 preserves embedded CMAF `tfdt` timestamps, while MPEG-TS transmux output is explicitly rebased onto the HLS playlist timeline.

...
tests/                 → Test suites
  integration.test.mjs → DB layer, auth, rate limiting, HTTP endpoints (71 tests)
...
public/                → Browser JS (plain JS, not TypeScript)
  ...
  native-player-engine.js → Native-first DASH/HLS/progressive player with the stable player adapter API, load-generation cancellation, bounded server-restart recovery, DASH sidx/template/list parsing, DASH/HLS image preview metadata, HLS fMP4/MPEG-TS support, HLS session-data chapters, and embedded CEA caption metadata
...
- **PWA offline mode**: `manifest.json` enables installability. SW caches `/offline` on install and caches `/downloads` + `/watch?v=*` pages + DASH MPD manifests on visit (network-first). All SW cache keys strip auth tokens via `stripToken()` so cached content is token-agnostic — segments cached during online play work offline with any/no token. Runtime JavaScript and `sw.js` are served with `Cache-Control: no-cache`, and player script URLs carry the current cache version, so an otherwise-fresh HTTP cache cannot pin an older player after deployment. SW marks navigation fallbacks with `X-SW-Fallback`; synthetic media cache-miss 503s include `X-SW-Offline` only when `WorkerNavigator.onLine` explicitly reports offline. The player also checks window `navigator.onLine`, allowing its network-hold/server-probe path to recover media requests after a server restart without confusing that state with genuine browser offline playback. Server probes have bounded request timeouts, held requests have a bounded recovery-cycle budget before returning persistent failures to provider recovery, transient token-refresh failures remain in recovery, and browser-managed HLS/progressive sources are reloaded with the current token after the server returns. `app.js` tracks `navigator.onLine` for the offline badge. Download metadata cached in `localStorage` for offline rendering. Offline bundle system: `GET /api/stream/:videoId/offline-bundle` (token-exempt) returns streamToken, MPD, title, channelTitle, format list, and `formatSizes`. `prepareForOffline(videoId)` in `app.js` fetches the bundle, sends it to the SW via `postMessage`, and downloads format files to IndexedDB (not Cache API). The SW's `message` handler builds a self-contained offline watch page HTML and caches it + the MPD + poster. `player.ts` falls back to `downloads` DB metadata when YouTube APIs fail (server-running-no-internet case). The `/offline` route requires no auth.
- **Player lifecycle and media timelines**: Every player load has a generation; unload, destroy, or a newer load invalidates stale asynchronous work before it can replace the active provider. Explicit VOD start times are clamped against manifest duration and applied after MSE metadata is usable. HLS fMP4 preserves embedded CMAF `tfdt` timestamps, while MPEG-TS transmux output is explicitly rebased onto the HLS playlist timeline. Independent audio/video initialization requests and SourceBuffer appends run concurrently so startup pays one initialization critical path rather than two. DASH and HLS ABR use transfer-duration-weighted fast/slow EWMAs, estimate and remove time-to-first-byte from throughput samples, keep separate upgrade/sustain hysteresis, serialize rendition initialization transitions, and automatically downshift when sampled decode-frame loss shows the active rendition is too expensive for the device.
- **Player QoE measurement**: First-party telemetry records load-to-first-composited-frame, play-intent-to-first-advancing-frame (video startup), navigation-to-first-frame (aggregate/page startup), play-to-playing as a separate event proxy, startup-buffer readiness, seeking-to-seeked latency, rebuffering, frame loss, quality switches, recovery, and network/cache behavior. It uses `requestVideoFrameCallback` where available and an animation-frame fallback, so `playing` is not mislabeled as a displayed frame. Optional watch-page oEmbed metadata and remote playlist context never hold the player document open: already-resolved cache data is retained, while remote details use a deduplicated 750ms background request. Initial details and playlist requests are queued after first playback plus an idle callback, with a 1.5s no-play fallback so they cannot occupy the first-frame critical path indefinitely.
- **Mux performance comparator**: `playwright.performance.config.mjs` and `tests/performance/player-parity.spec.mjs` run an isolated, pinned `@mux/mux-player` development comparator against the native engine. The 38-cell proof matrix covers both the production-preferred native DASH path and protocol-controlled HLS, using paired alternating cold contexts, cache/service-worker isolation, CDP network and CPU profiles, advancing-frame timing, hard quality/smoothness/duplicate-request gates, 200 startup pairs per cell (500 for seek), exact distribution-free quantile bounds, and 100,000 paired BCa resamples. Universal non-inferiority is an all-components-must-pass intersection-union test; optional native-better findings use a separate Bonferroni family-wise correction. Methodology and claim limits are documented in `docs/player-performance-parity.md`; comparator packages are development-only and are never part of the production player.
- **Live latency**: HLS startup honors advertised `HOLD-BACK` / `PART-HOLD-BACK`, falling back to three target durations (three part durations for LL-HLS). Ordinary live no longer starts on a 60-second delay or waits minutes to upgrade quality. The default HLS live forward/back buffers are 30s/8s, LL-HLS bounds startup/forward buffering to 1s/4s, and explicit player configuration still wins.
...
npm test                 # Integration tests (71 tests)
...
npm run benchmark:player   # Exploratory native-vs-Mux performance matrix
npm run benchmark:player:proof # 200 startup pairs/cell, 500 seek pairs, p50/p95 proof
npm run benchmark:player:audit # Independently recompute and verify canonical evidence
npm run test:all         # All test suites (71 + 23 + 12 + playlist parser tests)

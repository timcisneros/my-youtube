...
async function setupBaseData() {
  await db.upsertSubscriptions(TEST_USER, CHANNELS.map(ch => ({
    channelId: ch, title: `Channel ${ch}`, thumbnail: '', description: '',
  })));
  for (const ch of CHANNELS) {
    await db.setRssCache(ch, {
...
async function cleanupTestData() {
  // Clean subscriptions
  for (const ch of CHANNELS) {
    try { await db.deleteSubscription(TEST_USER, ch); } catch { /* ignore */ }
  }
  // Clean RSS cache — use raw SQL since there's no deleteRssCache in the API
  try {
    for (const ch of CHANNELS) {
      await db.setRssCache(ch, { channelTitle: '', items: [] });
    }
  } catch { /* ignore */ }
  // Clean watch times
  try {
    const watches = await db.getAllWatchTimesForUser(TEST_USER);
    for (const wt of watches) {
      // Reset watch time by setting to 0/0
      await db.setWatchTime(TEST_USER, wt.video_id, 0, 0);
    }
  } catch { /* ignore */ }
  // Clean dismissals, boosts, mutes, ratings, queue, topic filters, explore events
  try {
    const dismissed = await db.getDismissedVideoIds(TEST_USER);
    for (const vid of dismissed) await db.undismissVideo(TEST_USER, vid);
  } catch { /* ignore */ }
  try {
    const boosted = await db.getBoostedChannelIds(TEST_USER);
    for (const ch of boosted) await db.unboostChannel(TEST_USER, ch);
  } catch { /* ignore */ }
  try {
    const muted = await db.getMutedChannelIds(TEST_USER);
    for (const ch of muted) await db.unmuteChannel(TEST_USER, ch);
  } catch { /* ignore */ }
  try {
    const ratings = await db.getVideoRatings(TEST_USER);
    for (const r of ratings) await db.unrateVideo(TEST_USER, r.video_id);
  } catch { /* ignore */ }
  try {
    const queued = await db.getQueuedVideoIds(TEST_USER);
    for (const vid of queued) await db.unqueueVideo(TEST_USER, vid);
  } catch { /* ignore */ }
  try {
    const filters = await db.getTopicFilters(TEST_USER);
    for (const f of filters) await db.removeTopicFilter(TEST_USER, f.topic);
  } catch { /* ignore */ }
...
describe('Explore algorithm', () => {
  ...
  it('1. Empty state — no subs returns empty result', async () => {
  ...
  it('2. Basic result structure — has expected shape and bounds', async () => {
    await setupBaseData();
    await db.setDuration('EX_CH_A_v0', 321, 'not_live');
    const result = await getExploreVideos(TEST_USER);
    ...
    assert.strictEqual(result.durationSeconds.EX_CH_A_v0, 321);
    assert.strictEqual(result.liveStatuses.EX_CH_A_v0, 'not_live');
  });

  it('3. Watched channels rank higher than unwatched', async () => {
    await setupBaseData();
    // Watch several videos from CH_A (partial watch so they stay as candidates)
    for (let i = 0; i < 3; i++) {
      await db.setWatchTime(TEST_USER, `EX_CH_A_v${i}`, 200, 600);
    }
    cache.exploreVideos.delete(TEST_USER);
    const result = await getExploreVideos(TEST_USER);
  ...
  it('4. Continue watching removed — returns empty array', async () => {
    await setupBaseData();
    const result = await getExploreVideos(TEST_USER);
    assert.deepStrictEqual(result.continueWatching, []);
  });

  it('5. Channel mute exclusion — muted channel absent from output', async () => {
    await setupBaseData();
    await db.muteChannel(TEST_USER, 'EX_CH_B');
    cache.exploreVideos.delete(TEST_USER);
    const result = await getExploreVideos(TEST_USER);
  ...
  it('6. Dismissed videos excluded', async () => {
    await setupBaseData();
    await db.dismissVideo(TEST_USER, 'EX_CH_A_v0', 'EX_CH_A');
    cache.exploreVideos.delete(TEST_USER);
    const result = await getExploreVideos(TEST_USER);
  ...
  it('7. Filter bubble — max 2 per channel in top 10', async () => {
    await setupBaseData();
    // Give CH_A extreme affinity — 20 watches
    for (let i = 0; i < 5; i++) {
      await db.setWatchTime(TEST_USER, `EX_CH_A_v${i}`, 590, 600);
    }
    // Add more RSS items for CH_A so it has enough candidates
    await db.setRssCache('EX_CH_A', {
  ...
  it('8. New subscription boost — new channel videos appear', async () => {
    await setupBaseData();
    ...
    try {
      await db.deleteSubscription(TEST_USER, newCh);
      await db.setRssCache(newCh, { channelTitle: '', items: [] });
    } catch { /* ignore */ }
  });

  it('9. Cache hit — second call returns same reference', async () => {
    await setupBaseData();
    const result1 = await getExploreVideos(TEST_USER);
    const result2 = await getExploreVideos(TEST_USER);
    assert.strictEqual(result1, result2, 'Cached result should be same reference');
  });

  it('10. All videos have reason badges', async () => {
    await setupBaseData();
    const result = await getExploreVideos(TEST_USER);
    for (const v of result.videos) {
  ...
  it('11. Config override — zero affinity changes scoring', async () => {
    await setupBaseData();
    ...
    cache.exploreVideos.delete(TEST_USER);
    const normalResult = await getExploreVideos(TEST_USER);
    ...
    const modifiedResult = await getExploreVideos(TEST_USER, undefined, zeroAffinityConfig);
  ...
  it('12. Score normalization — videos are ordered sanely', async () => {
    await setupBaseData();
    // Watch some videos to generate meaningful signal
    await db.setWatchTime(TEST_USER, 'EX_CH_A_v0', 500, 600);
    await db.setWatchTime(TEST_USER, 'EX_CH_B_v0', 500, 600);
    cache.exploreVideos.delete(TEST_USER);
    const result = await getExploreVideos(TEST_USER);
